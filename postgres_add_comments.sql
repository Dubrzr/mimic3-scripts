...
COMMENT ON COLUMN ADMISSIONS.EDREGTIME is
   'Time that the patient was registered from the emergency department.';
COMMENT ON COLUMN ADMISSIONS.EDOUTTIME is
   'Time that the patient was discharge from the emergency department.';
COMMENT ON COLUMN ADMISSIONS.HOSPITAL_EXPIRE_FLAG is
...
COMMENT ON TABLE OUTPUTEVENTS IS
   'Events relating to substance output for patients.';
...
COMMENT ON COLUMN OUTPUTEVENTS.ISERROR is
   'Indicates when a caregiver observed an error with this event.';
...
COMMENT ON TABLE PROCEDUREEVENTS_MV IS
   '';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ROW_ID is
  'Unique row identifier.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.SUBJECT_ID is
  'Foreign key. Identifies the patient.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.HADM_ID is
  'Foreign key. Identifies the hospital stay.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ICUSTAY_ID is
  'Foreign key. Identifies the ICU stay.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.STARTTIME is
   'Date when the procedure started.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ENDTIME is
   'Date when the procedure ended.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ITEMID is
   'Foreign key. Identifies the charted item.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.VALUE is
   'Value of the event as a number.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.VALUEUOM is
   'Unit of measurement.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.LOCATION is
   '';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.LOCATIONCATEGORY is
   '';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.STORETIME is
   'Time when the event was recorded in the system.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.CGID is
   'Unique caregiver identifier.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ORDERID is
   'Identifier linking items which are grouped in a solution.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.LINKORDERID is
   'Identifier linking orders across multiple administrations. LINKORDERID is always equal to the first occuring ORDERID of the series.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ORDERCATEGORYNAME is
   'A group which the item corresponds to.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.SECONDARYORDERCATEGORYNAME is
   'A secondary group for those items with more than one grouping possible.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ORDERCATEGORYDESCRIPTION is
  'The type of item administered.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.ISOPENBAG is
  'Indicates whether the bag containing the solution is open.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.CONTINUEINNEXTDEPT is
  'Indicates whether the item will be continued in the next department where the patient is transferred to.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.CANCELREASON is
  'Reason for cancellation, if cancelled.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.STATUSDESCRIPTION is
  'The current status of the order: stopped, rewritten, running or cancelled.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.COMMENTS_EDITEDBY is
  'The title of the caregiver who edited the order.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.COMMENTS_CANCELEDBY is
  'The title of the caregiver who canceled the order.';
COMMENT ON COLUMN PROCEDUREEVENTS_MV.COMMENTS_DATE is
  'Time at which the caregiver edited or cancelled the order.';
